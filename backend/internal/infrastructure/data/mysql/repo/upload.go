...
func (r UploadRepository) CreateUpload(ctx context.Context, item uploadentity.Upload) (int64, error) {
	...
	result, err := r.db.ExecContext(ctx, `
		INSERT INTO uploads (actor_user_id, admin_user_id, original_filename, stored_path, file_size, status)
		VALUES (?, ?, ?, ?, ?, ?)
	`, record.ActorUserID, record.AdminUserID, record.OriginalFilename, record.StoredPath, record.FileSize, record.Status)
	if err != nil {
		return 0, err
	}
