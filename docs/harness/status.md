...
历史验证记录显示 `make test`、前端 `npm run build`、`make docker-build` 和 `BASE_URL=http://localhost:8001 make smoke` 已通过；本次复核里 `make smoke` 因本机后端可达性未能重新跑通。
...
- 本次复核里 `make smoke` 在本机 `8000/8001` 的可达性上遇到问题，未能重新跑通；当前完成态仍以仓库内既有 smoke 通过记录为准，后续在本机服务可达时建议再复跑一次。
